...
/*******************************************************************************
 * Port of sbseg.c by Tom Goldstein 2009 
 * 
 * This code performs isotropic segmentation using the "Split Bregman"
 * algorithm to Jave for integration into ImageJ.
 *
 ...
 * algorithm. An image of dimensions mxn is denoised using the following command
 * 
 * sbseg(f,edge,mu);
 * 
 * where:
 * 
 * - "f" is the mxn noisy image to be segmented - "mu" is the weighting parameter for the fidelity
 * term (mu should be about 1e-4 for images with pixels on the 0-255 scale) - "edge" is a 2D array
 * of edge detector values at each pixel (e.g. the weights for TV regularizer). For standard
 * segmentation simply choose "edge = ones(size(f))"
 */
...
public class SB_Segmentation implements PlugInFilter {
  ...
  public double[][] getEdges(int[][] pixels, int width, int height) {
    ...
    for (int y = yMin; y <= yMax; y++) {
      for (int x = xMax; x <= xMax; x++) {
         ...
         * double sum = 0.0;
         * 
         * if (Math.abs(sumX) < Math.abs(sumY)) sum = Math.abs(sumY); else sum = Math.abs(sumX);
         * 
         */
        sum = 1.0 / (1.0 + sum * sum);
        /*
         * if(sum>40) sum=0; else sum=1;
         */
  ...
  public double[] sbseg(double[][] f, double[][] edge, double[][] u, double[][] x, double[][] y,
      double[][] bx, double[][] by, double c1, double c2, double mu, double lambda, int nIter,
  ...
  public double gsUseg(double[][] f, double[][] u, double[][] x, double[][] y, double[][] bx,
      double[][] by, double c1, double c2, double mu, double lambda, int rows, int cols) {
    int r, c;
    ...
    for (c = 1; c < cols - 1; c++) {
      ...
      temp2 = c2 - f[r][c];
      g = temp1 * temp1 - temp2 * temp2;
      sum = x[r - 1][c] - bx[r - 1][c] + y[r][c - 1] - y[r][c] - by[r][c - 1] + by[r][c]
          - mu / lambda * g;
      u[r][c] = (u[r - 1][c] + u[r][c + 1] + u[r][c - 1] + sum) / 3.0;
      if (u[r][c] > 1)
        u[r][c] = 1;
    ...
    for (r = 1; r < rows - 1; r++) {
      ...
      temp2 = c2 - f[r][c];
      g = temp1 * temp1 - temp2 * temp2;
      sum = x[r - 1][c] - x[r][c] - bx[r - 1][c] + bx[r][c] + y[r][c - 1] - by[r][c - 1]
          - mu / lambda * g;
  ...
  public void gsSpaceseg(double[][] u, double[][] edge, double[][] x, double[][] y, double[][] bx,
      double[][] by, double lambda, int width, int height) {
    int w, h;
    double a, b, s;
